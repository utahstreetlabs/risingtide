...
(doseq [story [(with-meta (->TagLikedStory 1 2) {:timestamp 4})
               (with-meta (->ListingLikedStory 1 2 6 [3 4] [:ev :ylf]) {:timestamp 4})
               (with-meta (->ListingCommentedStory 1 2 6 [3 4] "hamburgers" [:ev :ylf]) {:timestamp 4})
               (with-meta (->ListingActivatedStory 1 2 6 [3 4] [:ev :ylf]) {:timestamp 4})
               (with-meta (->ListingSoldStory 1 2 6 [3 4] 5 [:ev :ylf]) {:timestamp 4})
               (with-meta (->ListingSharedStory 1 2 6 [3 4] :facebook [:ev :ylf]) {:timestamp 4})
               (with-meta (->ListingSavedStory 1 2 6 [3 4] 5 [:ev :ylf]) {:timestamp 4})
